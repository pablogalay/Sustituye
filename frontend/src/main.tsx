...
import {AddRounded,AnalyticsRounded,ArrowForwardRounded,CalendarMonthRounded,CheckCircleRounded,CloseRounded,DashboardRounded,DeleteForeverRounded,DescriptionRounded,DownloadRounded,HistoryRounded,MeetingRoomRounded,LogoutRounded,MenuRounded,PeopleAltRounded,SchoolRounded,ShieldRounded,StorageRounded,SyncRounded,UploadRounded,WarningAmberRounded} from '@mui/icons-material';
...
type Teacher={id:number;first_name:string;last_name:string;email:string;active:boolean;duty_weight:number};
...
type HallwayDuty={id:number;date:string;timeslot_id:number;post:string;label:string;teacher_id:number|null};
...
// FastAPI sends `detail` as a string for the errors this API raises itself, but as a list
// of validation objects on a 422; without unwrapping it the alert would read "[object Object]".
const errorDetail=async(r:Response):Promise<string|undefined>=>{try{const body=await r.json();if(typeof body.detail==='string')return body.detail;if(Array.isArray(body.detail))return body.detail.map((x:any)=>x.msg).filter(Boolean).join('. ')||undefined}catch{}return undefined};
const api=async(path:string,opts:RequestInit={})=>{const r=await fetch('/api'+path,{headers:{'Content-Type':'application/json',...(localStorage.token?{Authorization:`Bearer ${localStorage.token}`}:{ }),...opts.headers},...opts});if(!r.ok){throw new Error(await errorDetail(r)||`No se ha podido completar la acción (HTTP ${r.status})`)}return r.status===204?null:r.json()};
...
const apiDownload=async(path:string,fallbackName:string)=>{
  ...
  if(!r.ok){throw new Error(await errorDetail(r)||`No se ha podido completar la acción (HTTP ${r.status})`)}
...
function Teachers({teachers,reload}:{teachers:Teacher[];reload:()=>void}){
  const empty={first_name:'',last_name:'',email:'',password:'',active:true,duty_weight:'1'};
  const [form,setForm]=useState(empty),[reset,setReset]=useState<Record<number,string>>({}),[weight,setWeight]=useState<Record<number,string>>({}),[notice,setNotice]=useState(''),[error,setError]=useState('');
  const validWeight=(v:string)=>{const n=Number(v.replace(',','.'));return Number.isFinite(n)&&n>0};
  const save=async()=>{try{setError('');await api('/teachers',{method:'POST',body:JSON.stringify({...form,duty_weight:Number(form.duty_weight.replace(',','.'))})});setForm(empty);setNotice('Profesor añadido correctamente.');reload()}catch(e:any){setError(e.message)}};
  const changePassword=async(t:Teacher)=>{try{setError('');await api('/teachers/'+t.id,{method:'PUT',body:JSON.stringify({...t,password:reset[t.id]})});setReset({...reset,[t.id]:''});setNotice(`Contraseña actualizada para ${t.first_name}.`)}catch(e:any){setError(e.message)}};
  const updateWeight=async(t:Teacher)=>{const raw=weight[t.id];if(!validWeight(raw))return;try{setError('');await api('/teachers/'+t.id,{method:'PUT',body:JSON.stringify({...t,duty_weight:Number(raw.replace(',','.'))})});setWeight({...weight,[t.id]:''});setNotice(`Peso de guardia actualizado para ${t.first_name}.`);reload()}catch(e:any){setError(e.message)}};
  const deleteTeacher=async(id:number)=>{try{setError('');setNotice('');await api('/teachers/'+id,{method:'DELETE'});setNotice('Profesor eliminado correctamente.');reload()}catch(e:any){setError(e.message)}};
  return <Stack spacing={3}>
    <Paper sx={{p:{xs:2,sm:3},borderRadius:3}}>
      <Stack direction={{xs:'column',sm:'row'}} justifyContent="space-between" spacing={1} sx={{mb:2.5}}><Box><Typography variant="h6">Añadir profesor</Typography><Typography variant="body2" color="text.secondary">La contraseña debe tener al menos 8 caracteres. El peso de guardia multiplica lo que cuenta cada guardia realizada: usa más de 1 (por ejemplo 1,5) para quien trabaje a media jornada, así se le asignan menos.</Typography></Box><Chip label={`${teachers.length} cuentas`} color="primary" variant="outlined"/></Stack>
      <Box sx={{display:'grid',gridTemplateColumns:{xs:'1fr',sm:'repeat(2,1fr)',lg:'1fr 1fr 1.3fr 1.1fr 0.8fr auto'},gap:1.25,alignItems:'center'}}>
        <TextField label="Nombre" value={form.first_name} onChange={e=>setForm({...form,first_name:e.target.value})}/>
        <TextField label="Apellidos" value={form.last_name} onChange={e=>setForm({...form,last_name:e.target.value})}/>
        <TextField label="Correo electrónico" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/>
        <TextField label="Contraseña" type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})}/>
        <TextField label="Peso de guardia" type="number" inputProps={{step:0.1,min:0.1}} value={form.duty_weight} onChange={e=>setForm({...form,duty_weight:e.target.value})}/>
        <Button variant="contained" startIcon={<AddRounded/>} onClick={save} disabled={!form.first_name||!form.last_name||!form.email||form.password.length<8||!validWeight(form.duty_weight)}>Añadir</Button>
      </Box>
      {(notice||error)&&<Alert severity={error?'error':'success'} sx={{mt:2}} onClose={()=>{setError('');setNotice('')}}>{error||notice}</Alert>}
    </Paper>
    <Paper sx={{borderRadius:3,overflow:'hidden'}}>
      <Box sx={{p:2.5,borderBottom:'1px solid #eceef4'}}><Typography variant="h6">Cuentas del profesorado</Typography></Box>
      {teachers.length===0?<Empty icon={<PeopleAltRounded/>} text="Aún no se han creado cuentas de profesor."/>:<Stack divider={<Divider flexItem/>}>{teachers.map(t=>
        <Box key={t.id} sx={{p:{xs:2,sm:2.5}}}>
          <Stack direction={{xs:'column',md:'row'}} spacing={1.5} alignItems={{md:'center'}} justifyContent="space-between">
            <Stack direction="row" spacing={1.5} alignItems="center">
              <Avatar sx={{bgcolor:'#eef2ff',color:'primary.main',fontWeight:800}}>{t.first_name[0]}{t.last_name[0]}</Avatar>
              <Box>
                <Stack direction="row" spacing={1} alignItems="center">
                  <Typography fontWeight={750}>{t.first_name} {t.last_name}</Typography>
                  {t.duty_weight!==1&&<Chip size="small" color="secondary" variant="outlined" label={`Peso ${t.duty_weight}`}/>}
                </Stack>
                <Typography variant="body2" color="text.secondary">{t.email}</Typography>
              </Box>
            </Stack>
            <Stack direction={{xs:'column',sm:'row'}} spacing={1}>
              <TextField size="small" label="Peso de guardia" type="number" inputProps={{step:0.1,min:0.1}} sx={{width:{sm:130}}} value={weight[t.id]??String(t.duty_weight)} onChange={e=>setWeight({...weight,[t.id]:e.target.value})}/>
              <Button variant="outlined" onClick={()=>updateWeight(t)} disabled={!validWeight(weight[t.id]??String(t.duty_weight))||Number((weight[t.id]??String(t.duty_weight)).replace(',','.'))===t.duty_weight}>Guardar peso</Button>
              <TextField size="small" label="Nueva contraseña" type="password" value={reset[t.id]||''} onChange={e=>setReset({...reset,[t.id]:e.target.value})}/>
              <Button variant="outlined" onClick={()=>changePassword(t)} disabled={(reset[t.id]||'').length<8}>Actualizar</Button>
              <Tooltip title="Eliminar profesor"><IconButton color="error" onClick={()=>deleteTeacher(t.id)}><CloseRounded/></IconButton></Tooltip>
            </Stack>
          </Stack>
        </Box>
      )}</Stack>}
...
function Hallway({teachers,slots,currentTeacherId}:{teachers:Teacher[];slots:Slot[];currentTeacherId?:number}){
  const [day,setDay]=useState(todayIso());
  const [data,setData]=useState<{posts:{post:string;label:string}[];duties:HallwayDuty[]}>();
  const [error,setError]=useState('');
  useEffect(()=>{setData(undefined);setError('');api('/hallway-duties?day='+day).then(setData).catch((e:any)=>setError(e.message))},[day]);
  const teacherName=(id:number|null)=>{const t=teachers.find(x=>x.id===id);return t?`${t.first_name} ${t.last_name}`:'Sin cubrir'};
  const daySlots=data?slots.filter(s=>data.duties.some(d=>d.timeslot_id===s.id)).sort((a,b)=>a.period_number-b.period_number):[];
  const dutyAt=(slotId:number,post:string)=>data!.duties.find(d=>d.timeslot_id===slotId&&d.post===post)||null;
  const uncovered=data?data.duties.filter(d=>d.teacher_id===null).length:0;
  return <Stack spacing={3}>
    <Paper sx={{p:{xs:2,sm:3},borderRadius:3}}>
      <Stack direction={{xs:'column',sm:'row'}} spacing={2} alignItems={{sm:'center'}} justifyContent="space-between">
        <Box>
          <Typography variant="h6">Puestos de pasillo</Typography>
          <Typography variant="body2" color="text.secondary">Se asignan en orden (planta baja, primera y segunda) con el profesorado que queda libre tras las sustituciones.</Typography>
        </Box>
        <Stack direction="row" spacing={1.5} alignItems="center">
          <TextField size="small" label="Día" type="date" value={day} onChange={e=>setDay(e.target.value)} slotProps={{inputLabel:{shrink:true}}}/>
          {data&&<Chip size="small" color={uncovered?'warning':'success'} label={uncovered?`${uncovered} sin cubrir`:'Todo cubierto'}/>}
        </Stack>
      </Stack>
    </Paper>
    {error&&<Alert severity="error">{error}</Alert>}
    {!data?(error?null:<Loading/>):daySlots.length===0?<Paper sx={{borderRadius:3}}><Empty icon={<MeetingRoomRounded/>} text="Ese día no tiene sesiones lectivas."/></Paper>:
      <Paper sx={{borderRadius:3,overflow:'hidden'}}>
        <Box sx={{overflowX:'auto'}}>
          <Box component="table" sx={{width:'100%',borderCollapse:'collapse',minWidth:720,'th':{p:1.5,bgcolor:'#f8f9fc',fontSize:12,color:'text.secondary',fontWeight:800,whiteSpace:'nowrap',borderBottom:'1px solid #e9eaf2'},'td':{p:1.5,borderBottom:'1px solid #f0f1f5'},'td:first-of-type':{fontWeight:700,whiteSpace:'nowrap'}}}>
            <thead><tr><th style={{textAlign:'left'}}>Sesión</th>{data.posts.map(p=><th key={p.post} style={{textAlign:'left'}}>{p.label}</th>)}</tr></thead>
            <tbody>{daySlots.map(s=><tr key={s.id}><td>{slotRange(s)}</td>{data.posts.map(p=>{const duty=dutyAt(s.id,p.post);const mine=duty?.teacher_id===currentTeacherId&&currentTeacherId!==undefined;return <td key={p.post}><Chip size="small" variant={duty?.teacher_id?'filled':'outlined'} color={!duty?.teacher_id?'warning':mine?'success':'default'} label={teacherName(duty?.teacher_id??null)}/></td>})}</tr>)}</tbody>
          </Box>
        </Box>
      </Paper>}
  </Stack>
...
function Stats({admin,currentTeacherId}:{admin:boolean;currentTeacherId?:number}){
  ...
  return admin
    ? <Stack spacing={3}>
        <Paper sx={{p:{xs:2,sm:3},borderRadius:3,maxWidth:900}}>
          <Typography variant="h6">Carga de guardias</Typography>
          <Typography variant="body2" color="text.secondary" sx={{mb:3}}>Total ponderado por el peso de guardia de cada profesor/a (quien tiene un peso mayor de 1 acumula más carga por cada guardia realizada, así se le asignan menos).</Typography>
          {rows.length===0?<Empty icon={<AnalyticsRounded/>} text="Aún no hay datos estadísticos."/>:<Stack spacing={2}>{rows.map(x=>
            <Box key={x.teacher.id}>
              <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{mb:.65}}>
                <Stack direction="row" spacing={1} alignItems="center">
                  <Typography fontWeight={700}>{x.teacher.name}</Typography>
                  {x.teacher.duty_weight!==1&&<Chip size="small" variant="outlined" label={`Peso ${x.teacher.duty_weight}`}/>}
                </Stack>
                <Typography color="text.secondary" variant="body2">{round1(x.total)} sustituciones (pond.) · {round1(x.hallway_total)} pasillo</Typography>
              </Stack>
              <Box sx={{height:8,bgcolor:'#eef0f6',borderRadius:99,overflow:'hidden'}}><Box sx={{height:'100%',width:`${(x.total/maximum)*100}%`,bgcolor:'primary.main',borderRadius:99}}/></Box>
            </Box>
          )}</Stack>}
        </Paper>
        <DatabaseBackup/>
    ...
    : <Box sx={{maxWidth:340}}><StatCard label="Sustituciones realizadas" value={own?round1(own.total):0} detail="Total ponderado de coberturas que has hecho" tone="indigo" icon={<AnalyticsRounded/>}/></Box>;
}
